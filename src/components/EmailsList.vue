  ...
  <div class="list row d-flex justify-content-center">
    ...
    <div>
      ...
      <button class="m-3 btn btn-sm btn-danger">
        <router-link to="/add" class="nav-link" :style="{ color: 'white'}">Add</router-link>
      </button>
...
export default {
  ...
  methods: {
    ...
    removeAllEmails() { //delete all emails
      let res = this.resultReceiver;
      res = res.map(item => item.id)
      res.forEach(item => {
        EmailDataService.delete(item) //* get res done!
        .then(response => {
          console.log(response.data);
          this.refreshList();
        })
